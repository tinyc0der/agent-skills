...
`AGENTS.md` and `CLAUDE.md` at the repo root configure agents working on the [`tinyc0der/agent-skills`](https://github.com/tinyc0der/agent-skills) repository itself. When writing setup guides or docs, do not instruct users to copy these files into their own projects or into a global agent configuration; the reusable assets are the skills in `skills/`.
...
[Skill gap](https://github.com/tinyc0der/agent-skills/issues/new?template=skill-gap.yml)
