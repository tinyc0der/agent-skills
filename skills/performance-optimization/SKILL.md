...
**Do not cache:** anything whose staleness is a correctness bug (balances, permissions, inventory at checkout), or per-user data under a key that does not identify the user. See the optional supplemental `../../references/performance-checklist.md` for request coalescing, write strategies, negative caching, and the cache checklist.
...
The required measurement and optimization process is embedded above. Whole-pack installs can load the supplemental checklist at `../../references/performance-checklist.md`.
