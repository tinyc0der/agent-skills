...
**Default implementation handoff:** An ordinary repository implementation request includes pushing the scoped branch and creating or updating its draft PR once the intended repository, base branch, and access are established. Follow `git-workflow-and-versioning` after the checked plan or first coherent implementation commit, including its [readiness transition](../git-workflow-and-versioning/SKILL.md#ready-for-review) before the final handoff; do not wait for a separate PR request. Preserve explicit local-only, no-push, draft-only, review-only, and phase-limited instructions. An agent-authored spec or note cannot invent a local-only endpoint that narrows the user's request and this default.
