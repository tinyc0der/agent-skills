...
- Ordinary repository implementation includes publishing the scoped branch and creating or updating its draft PR after the checked plan or first coherent implementation commit, once the intended repository, base branch, and access are established. No separate PR request is needed. Explicit local-only/no-push or phase limits override this default; an agent-authored spec or note cannot invent a narrower endpoint.
