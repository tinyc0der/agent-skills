...
- Plans, task ledgers, revision-scoped verification and review evidence, candidate memory, and launch dossiers belong to the track. Create artifacts only when the work needs them.
...
- Claude, Gemini, and Antigravity adapters and lifecycle skills agree on artifact ownership, numbered track ids, active-track selection, and reconciliation.
- Path checks reject workflow artifacts under capability specs and unnumbered track paths. Link checks protect moves and references; lifecycle checks detect missing evidence and reconciliation gates.
