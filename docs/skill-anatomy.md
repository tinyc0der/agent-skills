...
The portability contract is explicit: a skill's required workflow, minimum checks, and exit criteria must remain in `SKILL.md`. Repo-root references are optional expanded guidance. A whole-repo install (such as the Claude Code marketplace plugin) carries them; a per-skill install that copies only `skills/<name>/` may omit them without blocking the skill. Every repo-root link in a skill must be labeled optional or supplemental so agents do not treat a missing pack-level file as a stop condition.

Current convention: material required by exactly one skill is a supporting file inside that skill's directory; material shared across skills goes in `references/` only after each consuming skill embeds the minimum required behavior.
