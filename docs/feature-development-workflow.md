...
Create only the files needed for the change. Each implementation track owns one focused PR, with an outcome, exclusions, acceptance evidence, dependencies/base, and a safe merge state. Check the whole PR before detailed tasks and when scope grows. Larger intent uses a coordination parent plus flat sibling tracks, even within one capability. Dependencies or a shared release do not require one huge PR: sequence tracks or use explicit stacked bases. A capability keeps one canonical spec across tracks; map Delivery cells select sections for one small PR or ordered child ids for several PRs. The parent `todo.md` indexes children, and its planning and integration docs PRs contain no child implementation. Follow the [delivery fork](../skills/using-agent-skills/SKILL.md#delivery-fork).

Allocate the next repository-wide three-digit track number above the highest existing prefix, starting at `001`, followed by a kebab-case name. Preserve ids and gaps and resolve concurrent allocation collisions before merge. Do not nest `docs/tracks/` or add `docs/epics/`.
