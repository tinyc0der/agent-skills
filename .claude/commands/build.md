...
`$ARGUMENTS` may narrow the task scope. An explicit request for one task or a stepwise checkpoint takes precedence over the default; do not treat an unknown argument as authorization for unrelated tasks. `/build` includes implementation, local commits, and the scoped draft-PR handoff through `git-workflow-and-versioning` once the intended repository, base branch, and access are established. Do not wait for a separate PR request. Explicit local-only, no-push, review-only, and phase-limited requests retain their limits; merge, deployment, and messages to other people require their own authorization.
...
1. **Resolve requirements and endpoint.** Use the selected workflow's requirement record, active numbered track, and linked capability specs. If the active track is an initiative (`role: initiative` or `docs/tracks/<track-id>/todo.md` indexes child track ids, even with leftover implementation tasks), do not implement those children here — open or resume the next unblocked child worktree using the allocated child track id. If the child's requirements are a stub, complete its selected route's prerequisites first; Feature children run `/spec` then `/plan` there and stop. A README or canonical spec alone does not authorize new behavior. Derive routine implementation choices from the actual request and contracts; ask only for a material unresolved intent or trade-off. Preserve an explicit single-task or read-only limit. Check the combined PR before each task. If it outgrows one focused review, preserve existing work and return to planning to split unmerged changes and remaining outcomes into tracks. Complete this track's verification and PR handoff before continuing authorized work in the next track's worktree. Auto mode cannot append another track to this PR; destructive migration contract needs a separate track and PR from expand.
...
7. **Request decisions and notify urgently.** Multiple approaches alone do not require a question; ask with options and a supported recommendation when consequential trade-offs remain unresolved by existing intent or delegation. Escalate missing access/approval, unresolved ownership conflicts, scope/cost/impact beyond authorization, and blockers or risks that cannot be resolved safely. Prepare the concrete decision, pause dependent work, and continue safe independent work while waiting. Evidence of an active production outage, suspected compromise, or ongoing data loss requires prompt notice to the user without waiting for diagnosis or preparation to finish. Stop unsafe actions and continue authorized containment and fixes; notification grants no additional authority or permission to contact others.
8. **Resume and finish.** After the user resolves a critical blocker, resume the authorized work without requiring another `/build` invocation. For ordinary implementation, publish the scoped branch and create or update its draft PR through `git-workflow-and-versioning`; return its URL and current revision with task evidence and limitations. If an explicit scope limit or verified access/destination/ownership blocker prevents that handoff, state the concrete reason and completed preparation instead of offering a PR later. In an end-to-end assignment, continue into verification and review within the agreed endpoint; an explicit build-only or `step` request ends at its specified checkpoint.
