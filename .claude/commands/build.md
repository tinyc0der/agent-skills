...
3. Apply `test-driven-development`'s test admission gate. For each materially changed contract or credible failure risk not already covered, write and run the smallest suitable failing behavior test (RED). If no new test is warranted because the change has no behavioral impact or existing coverage is sufficient, record why and identify the appropriate focused executable check; do not invent a test so the task has one.
