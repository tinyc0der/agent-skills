...
Before detailing the spec, apply the delivery fork in using-agent-skills to every request, including one capability. Keep a small coherent change on one track and PR. Split larger intent into child tracks with one reviewable PR outcome each; dependencies and a shared release do not prevent splitting. Map capabilities to track sections or ordered child ids. An initiative parent records outcomes and integration acceptance, leaving detailed requirements to its children.
