...
If the request bundles several independently testable capabilities, first propose a capability map (stable ids, dependency direction, build order, Delivery). Apply the delivery fork in `using-agent-skills`: Feature+map writes a section per capability in this track spec; an Epic parent writes initiative outcomes and leaves full child specs to child tracks.
