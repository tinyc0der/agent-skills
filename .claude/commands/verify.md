...
Verify the current completed change against its approved specification. Each implementation track verifies its one PR against its selected workflow, declared base, dependencies, and acceptance criteria. Return an oversized or bundled diff to planning before readiness. An initiative parent **planning** PR verifies artifact completeness (map, initiative spec, child index, stubs), not integration. An initiative parent **integration** PR verifies the assembled revision (remote default-branch head with required children merged); child PASS reports are citations, not a substitute.
