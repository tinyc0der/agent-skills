...
2. Resolve the numbered track under `docs/tracks/<track-id>/`; read its approved spec or bug report, linked `docs/specs/<capability>/spec.md` files, plan and task ledger when present, relevant commits, and existing PR body. One focused PR per implementation track. Check the whole diff against its declared base, outcome, and acceptance evidence; split oversized scope into tracks before expanding the PR. Dependent tracks may share a capability and release. Parent planning and integration docs PRs contain coordination and evidence only.
