...
The runner also copies the complete skill directory into `.eval-skill/` in the disposable workspace and tells the executor where to resolve supporting references. This tooling directory is excluded from the execution fixture's git status. Fixture paths retain their `files[]` prefix when copied, so prompts should name the project directory when its root is below the workspace.

Markdown under `evals/fixtures/` is test input, not repository documentation. The documentation-link validator excludes it because fixtures can intentionally contain broken links or use bundle-relative paths. Keep validating `evals/README.md` and other real documentation; behavioral expectations own the meaning of fixture links.
